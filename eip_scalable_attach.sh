...
set_associate_eip()
{
        ...
        echo "--${now}--" >> /home/ec2-user/operation/eip_auto_attach/eip_scalable_attach.log 2>&1
        # 割り当てEIPを書き込み
        echo ${available_alloc_id} >> /home/ec2-user/operation/eip_auto_attach/eip_scalable_attach.log 2>&1

        #
        # (3) 実行
        #- - - - - - - - - - - - -
        # no-allow-reassociationを指定することで上書きされることがなくなり、衝突した場合はエラー(true)が戻り値として返る
        aws ec2 associate-address --instance-id ${instance_id} --allocation-id ${available_alloc_id} --no-allow-reassociation >> /home/ec2-user/operation/eip_auto_attach/eip_scalable_attach.log 2>&1
